...
# source_2
nb_part_2 = 1000/ ui.number_of_threads
source = sim.add_source("GenericSource", "photon_source_2")
source.particle = "gamma"
source.n = nb_part_2
source.position.type = "box"
source.position.size = [0 * cm, 0 * cm, 0 * cm]
source.position.translation = [0 * cm, 0 * cm, -4 * nm]
source.direction.type = "momentum"
source.mother = world.name
source.direction.momentum = [0, 0, 1]
source.energy.type = "mono"
...
Phsp_act.output = output_path / "test062_KillActor.root"
...
f_phsp = uproot.open(output_path / "test062_KillActor.root")
arr = f_phsp["PhaseSpace"].arrays()
print("Number of detected events :", len(arr))
print("Number of expected events :", int(nb_part_2 * ui.number_of_threads))
# EventID = arr[0]

is_ok = len(arr) == nb_part_2 * ui.number_of_threads
gate.test_ok(is_ok)
